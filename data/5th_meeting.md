...
房价预测已经是我们的老朋友了，从一开始学习深度学习的举例时就用到了它，只是这一次输入的特征更多（不只有距离与面积），我们的目的时根据输入特征来预测房价输出，模型架构可由同学自由组合实现，因为我们当前主要学习有监督学习（使用深度学习实现），所以希望同学尽量使用神经网络模型，如果想用其他模型，可考虑集成学习。
...
* **原理** ：“三个臭皮匠，顶个诸葛亮”。
* **方法** ：
* **投票 (Voting)** ：训练多个不同的模型，分类时少数服从多数。
* **平均 (Averaging)** ：回归问题，取多个模型预测值的平均数（或者加权平均数，我们称为投票）。
